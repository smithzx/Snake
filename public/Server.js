class Server {
	...
	chooseHead(d, i) {
		...
		this.snakes[i].forEach(snake => {
			...
			cell.style["background-color"] = this.getColor(i);
		});
	}
		...
		for (let i = 0; i < this.players.length; i++) {
			try {
				...
				if (!step || !this.checkStep(i, step)) {
					...
					d.getElementById("result").style["background-color"] = this.getColor(i);
					d.getElementById("result").innerText = "Player #" + i
							+ " (" + this.players[i].constructor.name + ") - (" + this.getColor(i) + ") lose!\nstep:" +
		...
		for (let i = 0; i < this.players.length; i++) {
			this.snakes[i].forEach((snake, j) => {
				...
				if (j + 1 === this.snakes[i].length) {
					cell.style["background"] = "url(../head.png) no-repeat " + this.getColor(i);
					cell.style["background-size"] = "100%";
				} else {
					cell.style["background"] = "none";
					cell.style["background-color"] = this.getColor(i);
			...
			info += ("Player #" + i
					+ " (" + this.players[i].constructor.name + ") - (" + this.getColor(i) + ") length = " + this.lengths[i] + "\n");
		}
		if (this.intervalId) {
	...
	getCell(d, x, y) {
		return d.getElementById("cell" + x + "x" + y);
	}
...
(function (d) {
	let names = window.playerNames.split(",");
	let players = [];
	for (let i = 0; i < names.length; i++) {
		players.push(eval("new " + names[i].substring(0, names[i].lastIndexOf(".js")) + `(${i})`));
	}
	let server = new Server(players);
	...
	for (let i = 0; i < server.size * server.size; i++) {
		...
		cell.setAttribute("id", "cell" + x + "x" + y);
